...
CMD echo "{ \"authAudience\": \"$AUTH_AUDIENCE\", \"authClientID\": \"$AUTH_CLIENTID\", \"authDomain\": \"$AUTH_DOMAIN\", \"authRedirectUri\": \"$AUTH_REDIRECTURL\", \"cmsUrl\": \"$WEBAPI_ADDR\", \"featureToggleUrl\": \"$FEATURE_TOGGLE_URL\", \"mobileContentApiUrl\": \"$DATASTORE_URL\", \"achievementApiUrl\": \"$ACHIEVEMENT_URL\", \"thumbnailApiUrl\": \"$THUMBNAIL_URL\", \"docsUrl\": \"$DOCS_URL\", \"docsIntegrationUrl\": \"$DOCS_INTEGRATION_URL\"}" > hip-config.json && sed -i "s/<base href=\"\/\">/<base href=\"\/$SUB_DIR\/\">/g" index.html && sed -i "s/e.p=\"\/build\/\",/e.p=\"\/$SUB_DIR\/build\/\",/g" build/vendor.js && npm run serve
